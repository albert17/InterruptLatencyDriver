...
int configure_gpio_irq(void) {
   ...
   ret = gpio_request_one(test_data.gpio_pin, GPIOF_OUT_INIT_HIGH,
      DRV_NAME " gpio");
   if (ret < 0) {
   ...
   ret = gpio_request_one(test_data.irq_pin, GPIOF_IN, DRV_NAME " irq");
   if (ret < 0) {
   ...
   ret = gpio_to_irq(test_data.irq_pin);
   if (ret < 0) {
